...
#define PUTIN_STREAMDATA_NEEDKEYFRAME		0x04
...
typedef struct tagPostionResult_t     //移动物体的坐标和大小
{
	unsigned short		wX;
	unsigned short		wY;
...
typedef struct tagMotionRectList_t
{
	...
	PostionResult_t 	pPointList[1]; //有几个物体就有几个这种结构体,暂时只定义一个，这里的个数由 wImotion_num决定
}MotionRectList_t;

#define OTHERCMD_SETSUPPORTDIAREEK	0xFEFE
#define OTHERCMD_SETDEFOGPARAMS     0X0004 //设置透雾参数
#define OTHERCMD_SETDEFOGROI        0X0005 //设置透雾参数
...
//初始化类型
#define TMCC_INITTYPE_CONTROL				0x00//初始化成设备控制SDK
#define TMCC_INITTYPE_ENUM					0x01//初始化成枚举SDK
#define TMCC_INITTYPE_UPGRADE				0x02//初始化成升级SDK
#define TMCC_INITTYPE_TALK					0x03//初始化成语音对讲SDK
#define TMCC_INITTYPE_STREAM				0x04//初始化成播放数据流SDK
#define TMCC_INITTYPE_REALSTREAM			0x05//初始化成播放实时数据流SDK
#define TMCC_INITTYPE_LISTEN				0x06//初始化为状态接收和报警接收SDK
#define TMCC_INITTYPE_VIDEORENDER			0x07//初始化为视频显示SDK 							(just test)
#define TMCC_INITTYPE_VOICERENDER			0x08//初始化为语音对讲数据解码SDK 					(just test)
#define TMCC_INITTYPE_LISTENDEVICE			0x09//初始化为主动监听设备上传SDK 					(just test)
#define TMCC_INITTYPE_RTPSTREAM				0x0C//初始化为播放RTP包的SDK句柄 					(just test)
#define TMCC_INITTYPE_PLAYREMOTEFILE		0x0B//初始化为播放远程文件SDK句柄(通过回调读取数据)	(just test)
...
/* 2014-11-21 tzh 新加
 *函  数: TMCC_SetPlayWaveFileData
 *说  明: 播放内存中WAVE文件,通过语音对讲通道传送到DVS,DVS输出
 *参  数: hTmCC为控制句柄，pFileData为内存中文件数据,iFileSize为数据长度,bPlayNum为要播放的次数
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetPlayWaveFileData( HANDLE hTmCC, BYTE *pFileData, int iFileSize, unsigned char bPlayNum);
...
////////////////////////////////////////////////////////
/*
 *函  数: TMCC_RegisterRealStreamCallBack
 *说  明: 注册实时数据输出回调
 ...
 */
TMCC_API int TMCC_CALL TMCC_GetFileHeadInfo( HANDLE hTmFile, unsigned char* pHeadBuf, int iBufSize );
 ...
 * 返回值: 成功返回文件头长度，失败返回<=0
 */
TMCC_API int TMCC_CALL TMCC_CreateStreamHead( unsigned char* pHeadBuf, int iBufSize, tmStreamHeadInfo_t* pStreamHead );
 ...
 * 返回值: 成功返回文件头长度，失败返回<=0
 */
TMCC_API int TMCC_CALL TMCC_CreateFileHead( tmStreamHeadInfo_t* pStreamHead, unsigned char* pHeadBuf, int iBufSize );
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_GetDiareekCapability( HANDLE hTmCC, int* piMode );
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetImageProcessMode( HANDLE hTmCC, int iMode );


/*
 *函  数: TMCC_SetDiareekMode
 *说  明: 设置是否打开图像透雾功能
 *参  数: hTmCC为控制句柄,iMode为透雾开关，支持0-关，1-透雾模式1, 2-透雾模式2，3-透雾模式3
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetDiareekMode( HANDLE hTmCC, int iMode );
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API void TMCC_CALL TMCC_RegisterRtpStreamCallBack( HANDLE hClient, TMCC_RTPSTREAMREAD_CALLBACK pCallBack, void* context, BOOL bTcpPacket );
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_GetFaceDetectResult( HANDLE hTmCC, tmFaceDetectInfo_t* pResult );
